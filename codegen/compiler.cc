...
//std::vector<std::string>* Compiler::feedback_name_list =
//    new std::vector<std::string>(Compiler::init_feedback_name_list2());
...
void InstallUnoptimizedCode(UnoptimizedCompilationInfo* compilation_info,
  ...
  if (compilation_info->has_bytecode_array()) {
    ...
    /*StdoutStream os;
    os << shared_info->DebugNameCStr().get() << " " << shared_info->GetCode()
       << std::endl;*/
...
//static std::vector<std::string>* feedback_name_list_ = Compiler::init_feedback_name_list();
    //new std::vector<std::string>(Compiler::init_feedback_name_list());
//std::vector<std::string> Compiler::feedback_name_list = *feedback_name_list_;
    //*feedback_name_list = Compiler::init_feedback_name_list();

//void Compiler::init_feedback_name_list1() {
//  //std::vector<std::string> name_list;
//  std::string tmp_str;
//  std::ifstream in("D:\\master\\v8\\js_test\\ic_functions.txt");
//  while (getline(in, tmp_str)) {
//    feedback_name_list->push_back(tmp_str);
//  }
//}

std::vector<std::string> Compiler::init_feedback_name_list2() {
  std::vector<std::string> name_list;
  std::string tmp_str;
  std::ifstream in("D:\\master\\v8\\js_test\\ic_functions.txt");
  while (getline(in, tmp_str)) {
    name_list.push_back(tmp_str);
  }
  return name_list;
}
...
bool Compiler::Compile(Handle<JSFunction> function, ClearExceptionFlag flag,
  ...
  *is_compiled_scope = shared_info->is_compiled_scope(isolate);
  ...
  /*std::unique_ptr<char[]> debug_name = function->shared().DebugNameCStr();
  std::string name = debug_name.get();
  StdoutStream os;
  os << name << " " << function->shared().is_compiled() << std::endl;
  function->shared().PrintSourceCode(os);*/
  // ###################################################################
  
  /*vector<string> name_list;
  string tmp_str;
  ifstream in("D:\\master\\v8\\js_test\\ic_functions.txt");
  while (getline(in, tmp_str)) {
    name_list.push_back(tmp_str);
  }*/
  ...
  //std::string name = debug_name.get();
  //StdoutStream os;
  ////os << name << " " << function->shared().is_compiled() << endl;
  //if (find(feedback_name_list->begin(), feedback_name_list->end(), name) !=
  //    feedback_name_list->end()) {
  ...
  //    // for OSR. When we OSR functions with lazy feedback allocation we want
  //    // to
