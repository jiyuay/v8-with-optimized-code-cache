...
RUNTIME_FUNCTION(Runtime_MyRuntime) {
  SealHandleScope shs(isolate);
  DCHECK_EQ(2, args.length());

  //这是功能区，
  CONVERT_ARG_HANDLE_CHECKED(Code, code_target, 0);
  CONVERT_ARG_CHECKED(JSFunction, function, 1);
  StdoutStream os;

  /*std::unique_ptr<char[]> debug_name = function.shared().DebugNameCStr();
  char* name = debug_name.get();
  if (!strcmp(name, "son")) {
    os << "myruntime " << budget << " " << function << std::endl;
  }*/

  std::unique_ptr<char[]> debug_name = function.shared().DebugNameCStr();
  char* name = debug_name.get();
  if (!strcmp(name, "test")) {
    os << "myruntime " << code_target << " " << function << std::endl;
  }
...
RUNTIME_FUNCTION(Runtime_MyOptIc) {
  SealHandleScope shs(isolate);
  HandleScope scope(isolate);
  DCHECK_EQ(1, args.length());
  CONVERT_ARG_HANDLE_CHECKED(JSFunction, function, 0);
  ...
  string tmp_str;
  name_list.push_back("getfoo");

  std::unique_ptr<char[]> debug_name = function->shared().DebugNameCStr();
  string name = debug_name.get();
...
RUNTIME_FUNCTION(Runtime_MyOptTurbo) {
  SealHandleScope shs(isolate);
  DCHECK_EQ(1, args.length());
