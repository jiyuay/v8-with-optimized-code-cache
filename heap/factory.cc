...
Handle<JSFunction> Factory::JSFunctionBuilder::Build() {
  ...
  /*FILE* fp;
  fp = fopen("C:\\Users\\wzy\\Desktop\\zzz.txt", "a");

  OFStream os(fp);
  os << result->shared() << " " << result->shared().GetCode()
     << std::endl;
  fclose(fp);*/
  // ###################################################################################
  if (result->shared().is_feedbacked()) {
    if (!result->has_feedback_vector()) {
       result->raw_feedback_cell().set_interrupt_budget(
           FLAG_interrupt_budget);
      IsCompiledScope is_compiled_scope(
           result->shared().is_compiled_scope(result->GetIsolate()));
       JSFunction::EnsureFeedbackVector(result, &is_compiled_scope);
       result->feedback_vector().set_invocation_count(1);
    }
  }
  // ###################################################################################
...
Handle<JSFunction> Factory::JSFunctionBuilder::BuildRaw(Handle<Code> code) {
  ...
  /*StdoutStream os;
  os << sfi_ << " " << sfi_->is_feedbacked() << std::endl;*/
  /*StdoutStream os;
  os << sfi_->DebugNameCStr().get() << " " << code << std::endl;*/
