...
void InterpreterAssembler::CallJSAndDispatch(TNode<Object> function,
  ...
  // 这里的code_target为<Code BUILTIN Call_ReceiverIsAny>
  TNode<Code> code_target = HeapConstant(callable.code());
  //CallRuntime(Runtime::kMyRuntime, GetContext(), code_target, function);
  if (receiver_mode == ConvertReceiverMode::kNullOrUndefined) {
...
void InterpreterAssembler::UpdateInterruptBudget(TNode<Int32T> weight,
  ...
  TNode<JSFunction> function = CAST(LoadRegister(Register::function_closure()));
  // 如果函数在白名单里直接进入优化
  /*CallRuntime(Runtime::kMyRuntime, GetContext(), SmiFromInt32(weight),
              function);*/
  //CallRuntime(Runtime::kMyOptIc, GetContext(), function);
  ...
  //CallRuntime(Runtime::kMyRuntime, GetContext(), SmiFromInt32(old_budget), function);
  ...
  //CallRuntime(Runtime::kMyRuntime, GetContext(), SmiFromInt32(budget_after_bytecode), function);
